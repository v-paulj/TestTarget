...
| Internet (Client und Server) | ![Verfügbar im Manifest-Designer](images/ap-tools.png) | Bietet ein- und ausgehenden Zugriff auf das Internet und auf Netzwerke an öffentlichen Orten wie Flughäfen und Cafés. Diese Funktion ist eine Obermenge von **Internet (Client)**. **Internet (Client)** muss nicht aktiviert sein, wenn diese Funktion ebenfalls aktiviert ist. Der eingehende Zugriff auf kritische Ports ist immer blockiert. |
...
| Private Netzwerke (Client und Server) | ![Verfügbar im Manifest-Designer](images/ap-tools.png) | Bietet ein- und ausgehenden Zugriff auf Intranetnetzwerke, die über einen authentifizierten Domänencontroller verfügen oder vom Benutzer als Heim- oder Firmennetzwerke festgelegt wurden. Der eingehende Zugriff auf kritische Ports ist immer blockiert. | 
