# Entsperren von Windows mit Begleitgeräten
...
Falls das Begleitgerät über Biometrieunterstützung verfügt, kann das Begleitgeräteframework nur verwendet werden, wenn das Gerät mindestens eine der folgenden Bedingungen erfüllt:

- Es verfügt über eine Benutzeroberfläche und ein Windows-fremdes Betriebssystem, das den Benutzer durch eine biometrische Registrierung und Verwaltung führen kann (beispielsweise ein Smartphone mit Biometrie). Wenn ein Gerät unter Windows betrieben wird und Biometrie (Gesichts- und Fingerabdruckerkennung) unterstützt, muss es dem [Windows-Biometrieframework](https://msdn.microsoft.com/en-us/library/windows/hardware/mt608302(v=vs.85).aspx) entsprechen. 
- Das Gerät verfügt über keine Benutzeroberfläche (beispielsweise ein USB-Dongle) und verwendet keine Gesichts-, Iris- oder Fingerabdruckerkennung. 

Wenn keine dieser Bedingungen erfüllt ist und das Begleitgerät Biometrie unterstützt, halten Sie sich an die Richtlinien für [Windows Hello-App-Szenarien für OEMs und IHVs](https://msdn.microsoft.com/en-us/library/windows/hardware/mt608302(v=vs.85).aspx). 
...
- Eine aus dem Windows Store heruntergeladene, für das Begleitgerät bestimmte Begleitgeräte-App für die [universelle Windows-Plattform (UWP)](https://msdn.microsoft.com/en-us/windows/uwp/get-started/universal-application-platform-guide). 
...
Damit ein Begleitgerät in das Begleitgeräteframework eingebunden werden kann, muss es zunächst beim Framework registriert werden. Für den Registrierungsprozess ist allein die Begleitgeräte-App zuständig. Das Begleitgeräteframework schränkt diese UX in keiner Weise ein. 
...
- Etwas hat nicht geklappt. Melden Sie sich mit Ihrer PIN oder Ihrem Kennwort an, und richten Sie *Gerätename* erneut ein.
...
- Sprechen Sie Ihr Kennwort in *Gerätename*.
